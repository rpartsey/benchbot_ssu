...
def plot_fov_bounds(ax, origin, bounds):  
    ax.arrow(origin[0], origin[1], bounds[0][0], bounds[0][1], color='g')
    ax.arrow(origin[0], origin[1], bounds[1][0], bounds[1][1], color='b')
    
    
def plot_scan(ax, scan_points, **kwargs):
    result = ax.scatter(
        [x[0] for x in scan_points],
        [x[1] for x in scan_points], 
        **kwargs
    )
...
def plot_normals(ax, points, normals):
    for p, p_norm in zip(points, normals):
        ax.arrow(p[0], p[1], p_norm[0], p_norm[1], alpha=0.1)
